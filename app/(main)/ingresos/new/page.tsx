...
export default function NewTicketPage() {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    startTransition(async () => {
      // 1. Crear Ticket primero sin archivo (o con string vacío)
      ...
      // 2. Si hay archivo, subirlo usando el ID del ticket (o simplemente subirlo ahora)
      if (selectedFile) {
        try {
          ...
          // 3. Actualizar el ticket con la URL del archivo
          const { updateTicketInvoice } = await import("@/app/actions")
          await updateTicketInvoice({
            ticketId: result.ticketId,
            purchaseInvoiceUrl: publicUrl
          })

        } catch (error) {
          console.error("Error subiendo archivo:", error)
          alert("Ticket creado, pero hubo un error al subir la factura.")
        }
      }
                  ...
                  {!selectedFile ? (
                    ...
                    <div className="relative rounded-lg border p-4 flex flex-col items-center bg-muted/20">
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        className="absolute top-2 right-2 h-6 w-6 rounded-full bg-background border shadow-sm hover:bg-destructive hover:text-destructive-foreground z-10"
                      ...
                      {fileType === 'pdf' ? (
                        <div className="flex flex-col items-center gap-2 py-4">
                          ...
                          <p className="text-xs text-muted-foreground">PDF seleccionado</p>
                        </div>
                      ) : (
